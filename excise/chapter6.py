...
def idcheck():
	...
	while True:
		key = input("Please input your string  ")
		if key in keywords:
...
# 6-3
def sequence_sort():
	num = input("Please input numbers  ").split()
	nums = map(int,num)
	return sorted(nums)
...
def numtoeng():
    ...
    num = input("Enter a number  (0<num<1000)  ").lstrip('0')
...
# 6-11
def ip_convert():
    key = input("1.binary  2.decimal  ")
    if key == '1':
        ip = input("Enter your IP address of binary ")
        ip_list = ip.split('.')
        if len(ip_list) == 4:
        ...
        ip = input("Enter your IP adress of decimal  ")
        ip_list = ip.split('.')
...
def span():
    ...
    day3 = 0
    try:
        time1 = map(int,input("输入第一个时间：DD/MM/YYYY ").split('/'))
        time2 = map(int,input("输入第二个时间：DD/MM/YYYY ").split('/'))
...
# 用内置函数解决
def alive_day():
    time = map(int,input("Enter a date(YYYY/MM/DD) ").split('/'))
...
# 6-18
def form_out(list):
    num = int(input("Enter your colunm "))
